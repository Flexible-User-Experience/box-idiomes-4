...
export default class extends Controller {
    ...
    connect() {
        ...
        axios.get(Router.generate('admin_app_event_apiget', { id: this.eidValue }))
    ...
    update(event) {
        if (event.target.checked) {
            // attend class
            axios.get(Router.generate('admin_app_event_apiattendedclass', { id: this.eidValue, student: event.target.value}))
                .then(function (response) {
                })
                .catch(function (error) {
            ...
            // not atted class
            let self = this;
            axios.get(Router.generate('admin_app_event_apinotattendedclass', { id: this.eidValue, student: event.target.value}))
                .then(function (response) {
                    if (response.hasOwnProperty('data') && response.data.hasOwnProperty('said')) {
                        ...
                        sendEmailNotificationButton.setAttribute('href', Router.generate('admin_app_studentabsence_notification', {id: said}));
    ...
    studentAdded(event) {
        let self = this;
        axios.get(Router.generate('admin_app_event_apiattendedclass', { id: event.detail.event, student: event.detail.student}))
            .then(function () {
